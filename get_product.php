...
mysql_query("SET NAMES 'utf8'");	
	...
	if(isset($_SESSION['recipit_id']['New']) || isset($_SESSION['DRAFT']['delivery_id'])){
		...
		if($get_occs['occasion_name'] == 'Newly Born' || $get_occs['occasion_name'] == 'Expecting') { 
			$limit='LIMIT 0,250';
		} else {
			$get_data = get_category($gender,$age);
			$get_price = get_price($cash);
		...
		if(isset($_SESSION['LOGINDATA']['USERID'])){
 		 ...
 		 $query_demo = "select * from ".tbl_product." where proid = '".$category."' and (status = 1 or status = 0) $demographic $login_check2 $limit";
