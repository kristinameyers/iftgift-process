		...
		if($checkout_method == 'credit_card' || $checkout_methods == 'credit_card') {
			...
			if($charge->paid == '1') {
				...
				if($cardnumber == $card_number)
				{
					...
					$insQry =mysql_query("insert into ".tbl_checkout." set delivery_id	= '".$delivery_id."',
												...
												userId			= '".$userId."',
												payment_method  = '".$checkout_methods."',
			...
			if($recp_email)
			{
					...
					if($deliverd_status == 'pending') {	
						...
						$mail->From = 'Info@iftgift.com';
						$mail->FromName = 'iftGift';
						
						$mail->addAddress($recp_email, $recp_first_name);    													
						
						$mail->isHTML(true);                                
						$mail->Subject = 'It’s Fun Time -'.$giv_first_name.' has sent you an iftGift '.$email_subject;
						
						$mail->Body    = '<body style="font-family:Arial;">
			...
			{ 
				...
				if($deliverd_status == 'pending') {
					...
					$mail->isHTML(true);                                
					$mail->Subject = 'Hooray! Your iftGift to '.$recp_first_name.' is on its way';
				...
				$message .='<img alt='.$product['pro_name'].' src='.get_images($product['image_code']).' width="114" height="114" /></div>
...
/*print_r($_POST);exit;*/
	...
	if($delivery_id > 0){
		...
		if(mysql_num_rows($chkdraft) > 0){
		$upDate = mysql_query("update ".tbl_delivery." set step = 'checkout', draft = '1', dated = now() where delivery_id = '".$delivery_id."'"); 
		}
