<?php
	...
	if(isset($_SESSION['recipit_id']['New'])) { 
		...
		if(isset($_GET['catid'])) { 
			...
			$query_demo = "select * from ".tbl_product." where proid = '".$product_id."' and (status = 1 or status = 0) $login_check limit 0,200";
			$view_pro = $db->get_row($query_demo,ARRAY_A);
			$query_demos = "select * from ".tbl_product." where category = '".mysql_real_escape_string(stripslashes(trim($view_pro['category'])))."' and (status = 1 or status = 0) $login_check";
			$view_product = $view_pro['category'];
		...
		if(isset($_GET['catid'])) { 
			...
			$view_products = mysql_fetch_array($categorys);
			$view_product = $view_products['cat_name'];
			$query_demos = "select * from ".tbl_product." where category = '".mysql_real_escape_string(stripslashes(trim($view_products['cat_name'])))."' and (status = 1 or status = 0) and hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%' limit 0,200";
			$view_pro = $db->get_row($query_demos,ARRAY_A);
		} else if(isset($_GET['picId'])) {
			$product_id = mysql_real_escape_string(stripslashes(trim($_GET['picId'])));
			$query_demo = "select * from ".tbl_product." where proid = '".$product_id."' and (status = 1 or status = 0) and  hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%' limit 0,200" ;
			$view_pro = $db->get_row($query_demo,ARRAY_A);
			$query_demos = "select * from ".tbl_product." where category = '".mysql_real_escape_string(stripslashes(trim($view_pro['category'])))."' and (status = 1 or status = 0) and  hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%' limit 0,200";
			$view_product = $view_pro['category'];
		} else if(isset($_GET['scatid'])) {
			$category = mysql_real_escape_string(stripslashes(trim($_GET['scatid'])));
			$categorys = mysql_query("select cat_name from ".tbl_category." where catid = '".$category."' and p_catid != '0'");
			$view_products = mysql_fetch_array($categorys);
			$view_product = $view_products['cat_name'];
			$query_demos = "select * from ".tbl_product." where sub_category = '".mysql_real_escape_string(stripslashes(trim($view_products['cat_name'])))."' and (status = 1 or status = 0) and  hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%' limit 0,200";
			$view_pro = $db->get_row($query_demos,ARRAY_A);
		}
	}
/***********************GET NEXT PRODUCT********************************/
if(isset($_SESSION['recipit_id']['New']) || isset($_SESSION['DRAFT']['delivery_id'])) { 
	if(isset($_SESSION['LOGINDATA']['USERID'])){
		$login_check4="and hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%'";
	}
 $nxtsql = "SELECT proid FROM gift_product WHERE proid>{$view_pro['proid']} and (status = 1 or status = 0) $login_check4";
 ...
 $nxtsql = "SELECT proid FROM gift_product WHERE proid>{$view_pro['proid']} and (status = 1 or status = 0) and hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%'";
  $next_pro =  $nxtsql."ORDER BY proid LIMIT 1";
    $result = mysql_query($next_pro);
    if (@mysql_num_rows($result)>0) {
        $nextid = mysql_result($result,0);
...
if(isset($_SESSION['recipit_id']['New']) || isset($_SESSION['DRAFT']['delivery_id'])) { 
 ...
 $prevsql = "SELECT proid FROM gift_product WHERE proid<{$view_pro['proid']} and (status = 1 or status = 0) $login_check";
  $prev_pro =  $prevsql."ORDER BY proid DESC LIMIT 1";
	...
	$prevsql = "SELECT proid FROM gift_product WHERE proid<{$view_pro['proid']} and (status = 1 or status = 0) and hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%'";
  	$prev_pro =  $prevsql."ORDER BY proid DESC LIMIT 1";
    $results = mysql_query($prev_pro);
    if (@mysql_num_rows($results)>0) {
        $previd = mysql_result($results,0);
 ...
 <div class="overlayz" style="display:none"></div>
	...
	<?php if(isset($_SESSION['recipit_id']['New']) || isset($_SESSION['DRAFT']['delivery_id'])) { ?>
					...
					<?php } else if(isset($_SESSION['SHOPPRODUCT']) && $_SESSION['recipit_id']['New'] == '' && $_SESSION['DRAFT']['delivery_id'] == '' ){ ?>
						<!--<a class="singin" href="javascript:;" id="suggect_itemss">Purchase</a>-->
						<?php /*?><a href="<?php echo $affiliate_URLS; ?>"  target="_blank">Purchases</a><?php */?>
						<?php if($view_pro['img'] != '') { ?>
							<a href="<?php echo $view_pro['image_code']; ?>"  target="_blank">Purchase</a>	
						<?php }else{?>
							<a href="<?php echo $affiliate_URLS; ?>"  target="_blank">Purchase</a>	
...
jQuery.ajax({
    url: "<?php echo ru;?>process/get_catpro.php",
	type: "GET",
    dataType:'html',
	data:myData,
...
function get_product(pid)
{
	...
	var proId = pid;
	$.ajax({
		url: '<?php echo ru;?>process/get_catpro.php?picId='+proId,
		type: 'get', 
		success: function(output) {
