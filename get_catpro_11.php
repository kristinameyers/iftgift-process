<?php
	...
	if(isset($_SESSION['recipit_id']['New'])) { 
		...
		if(isset($_GET['catid'])) { 
			...
			$query_demos = "select * from ".tbl_product." where category = '".mysql_real_escape_string(stripslashes(trim($view_products['cat_name'])))."' and (status = 1 or status = 0) $login_check";
			...
			$query_demo = "select * from ".tbl_product." where proid = '".$product_id."' and (status = 1 or status = 0) $login_check";
			...
			$view_products = mysql_fetch_array($categorys);
			$view_product = $view_products['cat_name'];
			$query_demos = "select * from ".tbl_product." where sub_category = '".mysql_real_escape_string(stripslashes(trim($view_products['cat_name'])))."' and (status = 1 or status = 0) $login_check";
			$view_pro = $db->get_row($query_demos,ARRAY_A);
		}
	} else { 
		...
		if(isset($_GET['catid'])) { 
			...
			$view_products = mysql_fetch_array($categorys);
			$view_product = $view_products['cat_name'];
			$query_demos = "select * from ".tbl_product." where category = '".mysql_real_escape_string(stripslashes(trim($view_products['cat_name'])))."' and (status = 1 or status = 0) and hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%'";
			$view_pro = $db->get_row($query_demos,ARRAY_A);
		} else if(isset($_GET['picId'])) {
			$product_id = mysql_real_escape_string(stripslashes(trim($_GET['picId'])));
			...
			$view_pro = $db->get_row($query_demo,ARRAY_A);
			$query_demos = "select * from ".tbl_product." where category = '".mysql_real_escape_string(stripslashes(trim($view_pro['category'])))."' and (status = 1 or status = 0) and  hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%'";
			$view_product = $view_pro['category'];
		} else if(isset($_GET['scatid'])) {
			$category = mysql_real_escape_string(stripslashes(trim($_GET['scatid'])));
			$categorys = mysql_query("select cat_name from ".tbl_category." where catid = '".$category."' and p_catid != '0'");
	...
	?>
		<div class="sugget_left sugget_right">
			<div class="show_option">
				...
				?>
					<li onclick="get_product('<?php echo $allproducts['proid']; ?>')"><img src="<?php get_image($allproducts['image_code']); ?>" alt="<?php echo $allproducts['pro_name']; ?>" width="92" height="92" id="countButton_<?php echo $allproducts['proid']; ?>"/></li>
				<?php } } ?>	
				</ul>
			</div>
		</div>
